...
| S0-07 | Web shell: PKCE auth, scope context and banner, module registration, i18n loader, design tokens, component seeds | done 20 Sep in five steps (login; API client; scope banner, module definitions and navigation; design tokens, bundled fonts and the first three components; Playwright and the e2e stage); `docs/PROGRESS.md` | Scope switcher (M1 + K-02); step-up dialog (K-02); permission map (K-03); the nine other components (the module that needs them, on the base library of E-03) | 3 | Login → banner → hello page in three languages: proved by `make e2e` |
...
| S0-11 | CI pipeline; nightly fresh-clone test | done on `feat/s0-11-pipeline` (19 September), to be confirmed by its first run on GitHub and one nightly | Emulator smoke (after S0-08); publish (after a registry is chosen); coverage floors; Java format check | 1 | Green on `main`; nightly fresh-clone test |
| S0-12 | Hello module end to end: the proof table of 17A §12 | done on `feat/s0-12-hello-module` (19 September) except three proof rows | Till hello screen; till-side shared-engine parity test (the two-instance Playwright run of the proof table is done: S0-07 step 5) | 1.5 | All rows of the proof table pass |
...
### Decisions taken by the architect, 21 September 2026

The table above stays as 19A wrote it. These decisions sit on top of it and are what the lanes build to.

- **Spring Boot 3.5 comes before K-02.** Done: pull requests #55 and #58 put the repository on Spring Boot 3.5.16 and Spring Modulith 1.4.13 and took the two security patches ahead of it. K-02 adds the OAuth2 resource server on that line, not on 3.3.
- **A seed-backed permission resolver on the server, until M1-02 replaces it.** The permission catalogue and the role assignments are M1's data and do not exist yet, and the server checks no permission at all today: a hidden button in the web client is a courtesy and a direct request still succeeds. K-03b therefore resolves the identity provider's realm roles into permission sets from a seed file, with the same `PermissionResolver` interface the real resolver will have. M1-02 brings the catalogue and the seed goes.
- **K-02 arrives in two pull requests.** The first accepts a valid token **or** the development scope headers, so the web client, the Playwright suite and the twenty-eight integration tests keep passing while the claim mapping is proved; the Playwright suite, which signs in for real, is the net. The second deletes the headers, on both sides, together with `DevScopeArgumentResolver`, `DevCurrentScope` and `DevStubsGuard`.
- **The outbox relay is built twice.** First as a relay that polls the database and hands events to `BrokerAdapter`, which needs no broker and can be tested with Testcontainers alone; RabbitMQ is the second implementation, and it is accepted against the same tests. That is what "the broker is an assumption behind an interface" (AGENTS.md, doc 19 DR-2) has to mean in practice, and it takes the broker off the critical path.
- **The A4 PDF renderer waits for the first module that prints.** 19A §6 puts a headless Chromium worker inside K-06. Nothing prints before M4, the operational footprint is the open point of 19A §15 (DR-3), and the rest of K-06 — ICU messages with the fallback marker, NFC, collations and search, LATN digits — gates M2-02. The renderer moves to M4; K-06 is the rest.
- **K-03 splits.** **K-03a**: the idempotency table and the command interceptor order. It needs no M1 data, is buildable now, and closes a known hole: the 17A idempotency store is in memory, so it holds for one instance only and the two-instance stack already breaks it. **K-03b**: the permission resolver and its cache, the SoD helpers and the till snapshot signer.
- **K-13 splits.** **K-13a**: the clock, the per-device offset correction and the business date service. **K-13b**: the part driven by day-close, which needs till sessions, and is carried to M6.
- **K-08 starts alongside the till's S0-08**, with the thirteen conformance cases of doc 32 §11 as the joint gate: the same suite proves the gateway and the till against each other, and neither side is done until it is green.
- **Start order: K-01, K-04, K-03a.** Row-level security first, because every later migration is written against the policy template; then audit, because every handler already calls `audit.record` and the assertion is still counting stub calls; then the idempotency table.
- **The estimate is 45 to 55 developer-days, not the guide's 27.** An estimate, not a measurement, and made on the state of the repository on 21 September: K-02 is 5 to 6 days, not 3 (two pull requests, the provider mapper endpoint, and the development headers to remove on both sides); K-05 is 5, not 3 (two relay implementations and the two-instance exactly-once test); K-08 is 9 to 10, not 5 (the thirteen conformance cases and the till simulator are most of the ticket). Doc 20 carries a reserve; this is the second measured correction after 19A §14's own and should be recorded there (P-04).
- **One risk, unverified, is the first task of K-04.** `audit_event`, `event_outbox` and the document tables are partitioned by month, and `PartitionJob` creates the next partitions three months ahead. Whether PostgreSQL applies the parent's `ENABLE ROW LEVEL SECURITY`, `FORCE ROW LEVEL SECURITY` and policies to a partition created later, or whether the job has to set them on each new partition itself, **has not been verified on PostgreSQL 16**. If it does not, `SchemaRulesIntegrationTest` fails at the first month rollover and, worse, the new partition is readable across entities until it does. K-04 answers it with a test before it writes the partition job.
...
| Web | S0-07 done (verified against 17A §9 and doc 30, and by `make e2e`); next web work is the M1 and M2 screens | compose stack for the PKCE login |
...
| D language and sync | K-06 messages, collation and formats (the A4 renderer moved to M4 by the decision of 21 September), then K-08 sync gateway with the till developer | 7.5 | K-08 needs device auth from K-02, the snapshot signer from K-03 and the change-log fan-out from K-05; enrolment, ingestor and simulator start against the stubs |
...
| Kernel migrations | Two lanes pick the same version number | Reserved ranges per lane, written into `backend/app/src/main/resources/db/migration/kernel/README.md` and settled on 21 September 2026: V0010–V0029 lane A security (K-01, K-02, K-03), V0030–V0049 lane B ledgers and events (K-04, K-05, K-10), V0050–V0079 lane C configuration, documents and jobs (K-11, K-12, K-13, K-07, K-09), V0080 and up lane D language and sync (K-06, K-08). Below V0010 stays free for the baseline. Module migrations live in their own directories and each starts again at V0001. The lanes may merge in any order: the local stack runs Flyway with `MIGRATION_OUT_OF_ORDER=true`, while the application's default stays strict for deployed environments (decided 21 September 2026; the README gives the measurement and the reasoning) |
