...
| S0-05 | ArchitectureTests and pipeline scripts (schema ownership, i18n completeness) | partly | Remove or justify the extra kernel-internals rule; wire the two scripts into `make verify`; i18n files are still empty | 1 | A deliberate violation fails the build |
| S0-06 | OpenAPI-first toolchain: generator config, `hello.yaml`, controller stub, `gen-clients` | partly | Backend interface generation from the slice; stale-generated check in CI | 1 | Client generated; stale check works |
...
| S0-10 | Local compose: PostgreSQL, PgBouncer, RabbitMQ, MinIO, Keycloak, Mailpit; dev realm; seeds | merged, unverified (`compose.yml`, `compose.two.yml`, `realm-dev.json`, `nginx-two.conf`) | Check the service list against 17A §8; seed scripts; run `make up` and `make up-2` on a fresh clone | 1.5 | `make up` and `make up-2` pass |
...
| S0-12 | Hello module end to end: the proof table of 17A §11 | partly (handler, queries, migration, controller and web page merged in pull requests 15 and 16) | Till hello screen; integration tests; run every row of the proof table | 1.5 | All rows of the proof table pass |
