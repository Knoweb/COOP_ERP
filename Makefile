...
test-scaffold:
	...
	node tools/new-module.mjs --name m99dummy --schema dummy --entity dummy_item
	sh tools/gen-clients.sh
	...
	grep -rl "todo\.dummy\.dummy_item\." backend/app/src web/src | xargs sed -i "s/todo\.dummy\.dummy_item\./dmy.dummy_item./g"
	node tools/check-permissions.mjs
