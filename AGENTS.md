...
- **Build order is fixed.** Sprint 0 first (17A tickets S0-01…S0-13, then 19A K-01…K-13). Modules run in parallel with the kernel, one developer per module (decided by the architect, 24 September 2026), and each module ticket names the kernel ticket it waits for; the hello module's integration tests stay green throughout, against the stubs until the real service lands. Within a module, follow the nA guide's section 10 ticket order.
...
- **Every handler test asserts what was audited and published.** Integration tests extend `PostgresIntegrationTest` and inherit the field `kernel` (`KernelRecorder`): assert `kernel.committedAudit()` and `kernel.committedEvents()` against the audit event and domain event that the handler specification names (nA guide, section 6), and for a failure case that nothing was committed. The build already fails when a `@CommandHandler` never calls `audit.record` and `events.publish`, or when anything but a command handler writes to the database; those rules prove the calls exist, only the test proves they are right. The hello module (`hello/README.md`) is the worked example.
...
- **Say `make reset` in the pull request body when your change needs it.** A migration runs on the next start against any database, and so does the role script (`infra/compose/postgres/init/01-roles.sh`, idempotent, run by `make up`), so a new database user or membership needs no reset. What does need one: a renamed or removed migration, which Flyway refuses on an old database ("applied migration not resolved locally"). The author knows; the reader should not have to guess. Everybody starts the day with `make sync` and reads the merged pull requests for that line. A login user is created by the role script, never by a migration: its password is environment work, and PostgreSQL 16 lets the migrator create a role but not add it to a group it did not create.
