...
- **Say `make reset` in the pull request body when your change needs it.** A migration runs on the next start against any database; but a change to what runs at database creation (`infra/compose/postgres/init`), or a renamed or removed migration, only takes effect on a recreated database, and Flyway refuses an old one ("applied migration not resolved locally"). The author knows; the reader should not have to guess. Everybody starts the day with `make sync` and reads the merged pull requests for that line.
