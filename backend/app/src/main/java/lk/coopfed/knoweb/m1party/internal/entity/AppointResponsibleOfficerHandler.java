...
import lk.coopfed.knoweb.m1party.api.ResponsibleOfficerAppointed;
...
class AppointResponsibleOfficerHandler implements Handles<AppointResponsibleOfficer, UUID> {
    ...
    static final String AUDIT_RESPONSIBLE_OFFICER_APPOINTED = "RESPONSIBLE_OFFICER_APPOINTED";

    private final EntityRepository repository;
    private final ResponsibleOfficerOwnership ownership;
    private final AuditFacade audit;
    private final EventPublisher events;

    AppointResponsibleOfficerHandler(
            EntityRepository repository,
            ResponsibleOfficerOwnership ownership,
            AuditFacade audit,
    ...
    public UUID handle(AppointResponsibleOfficer command, ScopeContext scope) {
        ...
        if (!ownership.belongsToEntity(command.userId(), entity.getId())) {
            throw new ProblemException(
        ...
        audit.record(AUDIT_RESPONSIBLE_OFFICER_APPOINTED, Subject.of("entity", entity.getId()), before, after, scope);

        events.publish(new ResponsibleOfficerAppointed(
                entity.getId(), entity.entityCode(), entity.entityType(), entity.status()));
