...
/**
 * The greeting aggregate. Rules every entity in the system follows:
 * <ul>
 *   <li>{@code @Table} names the module's own schema; the architecture test R4 fails otherwise</li>
 *   <li>no setters: a greeting is created once and never changed (the application's database
 *       role has no UPDATE grant on the table anyway)</li>
 *   <li>the entity stays inside {@code internal}; callers get a {@link GreetingView}</li>
 * </ul>
 *
 ...
 */
@Entity
@Table(schema = "hello", name = "greeting")
public class Greeting implements Persistable<UUID> {
    ...
    private Greeting(UUID id, UUID ownerEntityId, String textEn, String textSi, String textTa) {
        this.id = id;
    ...
    /** The only way to make a greeting. Blank translations are stored as null ("not translated"). */
    static Greeting create(UUID id, UUID ownerEntityId, String textEn, String textSi, String textTa) {
        return new Greeting(id, ownerEntityId, textEn.strip(), blankToNull(textSi), blankToNull(textTa));
    }

    /**
     * The read-only copy handed to callers, and the "after" state recorded in the audit log.
     * createdAt is set by the database, so it is null until the greeting has been read back.
     */
    GreetingView snapshot() {
        return new GreetingView(id, ownerEntityId, textEn, textSi, textTa, status, createdAt);
    }
